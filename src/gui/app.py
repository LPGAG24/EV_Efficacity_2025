...
ev_share = st.sidebar.slider("EV share (%)", 0, 100, 10)
car_count = int(total_vehicles * ev_share / 100)
...
home_mu = st.sidebar.slider("Home peak hour", 0.0, 23.0, 18.0)
home_sigma = st.sidebar.slider("Home σ", 0.5, 5.0, 2.0, step=0.5)
work_mu = st.sidebar.slider("Work peak hour", 0.0, 23.0, 9.0)
work_sigma = st.sidebar.slider("Work σ", 0.5, 5.0, 2.0, step=0.5)
...
if home_only:
    ...
    home_share = st.sidebar.slider("Home charging %", 0, 100, 70) / 100
